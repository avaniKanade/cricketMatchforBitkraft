    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.matches.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start_game') ? 'is-invalid' : '' }}" type="text" name="start_game" id="start_game" value="{{ old('start_game') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('result_1') ? 'is-invalid' : '' }}" type="number" name="result_1" id="result_1" value="{{ old('result_1', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('result_2') ? 'is-invalid' : '' }}" type="text" name="result_2" id="result_2" value="{{ old('result_2', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('team_1') ? 'is-invalid' : '' }}" name="team_1_id" id="team_1_id">
                    @foreach($team_1s as $id => $entry)
                        <option value="{{ $id }}" {{ old('team_1_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('team_2') ? 'is-invalid' : '' }}" name="team_2_id" id="team_2_id">
                    @foreach($team_2s as $id => $entry)
                        <option value="{{ $id }}" {{ old('team_2_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
